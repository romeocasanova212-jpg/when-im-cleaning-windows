...
3. **Monetization → Products → In-app products**
...
Debug Mode: ✓ (checked)
Enable IAP: ✓ (checked)
Validate Receipts: ✗ (unchecked for debug)
```
...
## 📱 Restore Purchases (iOS Required)
...
**Solution**: Unity IAP package not installed or services not enabled. Check **Services → In-App Purchasing**.
...
- Upgrade path: Bronze → Silver → Gold
